...
1. **IntentRouter** detects: `[HEALTH:symptom_check, MALL:product_search]`
2. **CollaborationManager** builds dependency graph: `MALL depends on HEALTH` (allergy info needed)
3. **HealthAgent** runs first → publishes findings to AgentBus
4. **MallAgent** reads health context from AgentBus → searches hypoallergenic products
5. **ConflictResolver** finds no conflict → **ResultAggregator** merges output into a natural reply
